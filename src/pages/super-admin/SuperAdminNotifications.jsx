...
function SuperAdminNotifications() {
  ...
  const handleViewDetails = (notification) => {
    ...
    if (notification.category === 'University Management' || notification.category === 'Licensing') {
      navigate('/super-admin/universities');
    } else if (notification.category === 'Platform Administration') {
      navigate('/super-admin/settings');
    } else if (notification.category === 'Security') {
      navigate('/super-admin/settings'); // Assuming security is under settings
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header" style={{ flexDirection: 'column', alignItems: 'flex-start', padding: '20px 30px', height: 'auto' }}>
        <h2 style={{ fontSize: '24px', margin: '0 0 8px 0' }}>Global Notifications</h2>
        <p style={{ margin: 0, color: '#6c757d', fontSize: '0.95rem' }}>
          Monitor platform-wide activity, system health, and university status.
        </p>
      </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
              <span className="badge category" style={{ fontSize: '0.8rem', padding: '4px 8px', background: '#f1f5f9', borderRadius: '4px', color: '#475569' }}>
                {selectedAnnouncement.category}
              </span>
              <span className={`badge priority-${selectedAnnouncement.priority.toLowerCase()}`} style={{ fontSize: '0.8rem', padding: '4px 8px', borderRadius: '4px', fontWeight: '500' }}>
            ...
            <div style={{ color: '#475569', lineHeight: 1.6, padding: '15px 0', borderTop: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0' }}>
            ...
            <div style={{ marginTop: '20px', color: '#64748b', fontSize: '0.9rem' }}>
              <strong>Suggested Action:</strong> {
                selectedAnnouncement.category === 'System' 
                  ? 'Review system logs in the infrastructure dashboard.' 
                  : selectedAnnouncement.category === 'Security'
                  ? 'Check audit logs and verify administrator access.'
